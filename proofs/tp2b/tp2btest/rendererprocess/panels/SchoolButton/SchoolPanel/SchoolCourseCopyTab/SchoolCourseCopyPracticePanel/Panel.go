...
func NewPanel(ctx context.Context, ctxCancel context.CancelFunc, receiveChan lpc.Receiving, sendChan lpc.Sending, help *paneling.Help) (panel *Panel, err error) {
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf("SchoolCourseCopyPracticePanel: %w", err)
		}
	}()
	...
	presenter := &panelPresenter{
		group:          group,
		tabPanelHeader: document.ElementByID("mainMasterView_home_pad_SchoolButton_SchoolPanel_tab_bar-SchoolCourseCopyTabPanel-H3"),
		tabButton:      document.ElementByID("mainMasterView_home_pad_SchoolButton_SchoolPanel_tab_bar-SchoolCourseCopyTab"),
	}
	messenger := &panelMessenger{
		group: group,
	}
