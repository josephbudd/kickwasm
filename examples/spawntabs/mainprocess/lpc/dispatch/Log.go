...
import (
	...
	"github.com/josephbudd/kickwasm/examples/spawntabs/domain/data/loglevels"
...
func handleLog(ctx context.Context, rxMessage *message.LogRendererToMainProcess, sending lpc.Sending, errChan chan error) {
	...
	case loglevels.LogLevelInfo:
		msg = "spawntabs: Info: " + rxMessage.Message
	case loglevels.LogLevelWarning:
		msg = "spawntabs: Warning: " + rxMessage.Message
	case loglevels.LogLevelError:
		msg = "spawntabs: Error: " + rxMessage.Message
		err = errors.New(msg)
	case loglevels.LogLevelFatal:
		msg = "spawntabs: Fatal: " + rxMessage.Message
		err = errors.New(msg)
	default:
		msg = fmt.Sprintf("spawntabs: %d: %s", rxMessage.Level, rxMessage.Message)
	}
