...
func NewDOM(eventHandlerID uint64) (d *DOM) {
	d = &DOM{
...
func (d *DOM) ElementByID(id string) (el *markup.Element) {
	...
	el = markup.NewElement(e, d.eventHandlerID)
	return
}
...
func (d *DOM) ElementsByTagName(tagName string) (els []*markup.Element) {
	...
	for i := 0; i < l; i++ {
		els[i] = markup.NewElement(ee.Index(i), d.eventHandlerID)
	}
	return
}


// ElementsAt returns all of the elements at x, y.
// The slice begins with the top most element and ends with the html element.
func (d *DOM) ElementsAt(x, y float64) (els []*markup.Element) {
	ee := document.Call("elementsFromPoint", x, y)
	l := ee.Length()
	els = make([]*markup.Element, l)
	for i := 0; i < l; i++ {
		els[i] = markup.NewElement(ee.Index(i), d.eventHandlerID)
...
func (d *DOM) NewElementFromJSValue(jsv js.Value) (el *markup.Element) {
	el = markup.NewElement(jsv, d.eventHandlerID)
	return
}
