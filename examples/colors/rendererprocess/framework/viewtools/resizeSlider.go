			...
			for i2 := 0; i2 < l2; i2++ {
				...
				if !userContentClassList.Call("contains", UnSeenClassName).Bool() {
					if userContentClassList.Call("contains", UserContentClassName).Bool() {
						...
						log.Printf("markupPanel #%s", markupPanel.Get("id").String())
						w -= window.WidthExtras(markupPanel)
						h -= window.HeightExtras(markupPanel)
						window.SetStyleWidth(markupPanel, w)
						...
						for i4 := 0; i4 < templateElementsLength; i4++ {
							...
							if !classList.Call("contains", UnSeenClassName).Bool() {
								if classList.Call("contains", ResizeMeHeightClassName).Bool() {
									...
									// 1. Don't allow vertical scrolling in the user content panel.
									userContentClassList.Call("remove", VScrollClassName)
									// 2. Set the height of the markup panel.
									window.SetStyleHeight(markupPanel, h)
									break
								}
							}
						}
						// Template element sizes.
						for i4 := 0; i4 < templateElementsLength; i4++ {
							el := templateElements.Index(i4)
							classList := el.Get("classList")
							if !classList.Call("contains", UnSeenClassName).Bool() {
								if classList.Call("contains", ResizeMeWidthClassName).Bool() {
									resizeTemplateMarkupWidth(el, w)
								}
								if classList.Call("contains", ResizeMeHeightClassName).Bool() {
									log.Printf("resizeTemplateMarkupHeight(#%s, %f)", ch.Get("id").String(), h)
									resizeTemplateMarkupHeight(el, h)
								}
								h -= window.OuterHeight(el)
							}
						}
						return
					}
