package templates

// BuildPackDotSH is the shell script that builds and packs the renderer.
const BuildPackDotSH = `#!/bin/bash

# Paths.
startwd=$PWD
appwd="${PWD%/*}"
prefix="${appwd##*/}"
authorwd="${appwd%/*}"
step=0

sitepack_package_name="${prefix}sitepack"
sitepack_output_package_path="${authorwd}/${sitepack_package_name}"

spawnpack_package_name="spawnpack"
spawnpack_package_path="${appwd}/renderer/${spawnpack_package_name}"
spawn_templates_folder="spawnTemplates"
spawnpack_templates_path="${appwd}/{{.SiteFolder}}/${spawn_templates_folder}"

# Remove the old spawn package if it's there.
if [ -d "${spawnpack_package_path}" ]
then
    echo ""
    (( step += 1 ))
    echo "STEP ${step}:"
    echo "REMOVE YOUR PREVIOUS BUILD OF ${spawnpack_package_path}"
    echo "rm -r ${spawnpack_package_path}"
    rm -r "${spawnpack_package_path}"
fi

# Write the new spawn package source code.
# This is for the renderer so it must be a file store not a reader.
echo ""
(( step += 1 ))
echo "STEP ${step}:"
echo "WRITE THE SOURCE CODE FOR YOUR NEW ${spawnpack_package_name} PACKAGE."
echo " * The ${spawnpack_package_name} package is your renderer's spawn html templates."
echo "cd ${appwd}/{{.SiteFolder}}"
cd "${appwd}/{{.SiteFolder}}"
echo "kickpack -nu -o=${spawnpack_package_path} ./${spawn_templates_folder}"
kickpack -nu -o="${spawnpack_package_path}" "./${spawn_templates_folder}"
if [ $? -gt 0 ]
then
    echo "Oops! Job.Ended."
    exit
fi
echo "cd ${startwd}"
cd "${startwd}"

echo " * Success. The source code for your new ${spawnpack_package_name} package is written."

# Build the wasm.
echo ""
(( step += 1 ))
echo "STEP ${step}:"
echo "BUILD THE RENDERER GO CODE INTO WEB ASSEMBLY CODE AT ../{{.SiteFolder}}/{{.AppDotWASM}}"
echo "GOARCH=wasm GOOS=js go build -o ../{{.SiteFolder}}/{{.AppDotWASM}} {{.MainDotGo}} {{.PanelsDotGo}}"
GOARCH=wasm GOOS=js go build -o ../{{.SiteFolder}}/{{.AppDotWASM}} {{.MainDotGo}} {{.PanelsDotGo}}
if [ $? -gt 0 ]
then
    echo "Oops!"
    exit
fi

echo " * Success. The renderer go code is compiled into web assembly code at ../{{.SiteFolder}}/{{.AppDotWASM}}"
    
# Remove the old sitepack package if it's there.
if [ -d "${sitepack_output_package_path}" ]
then
    echo ""
    (( step += 1 ))
    echo "STEP ${step}:"
    echo "REMOVE YOUR PREVIOUS BUILD OF ${sitepack_package_name}"
    echo "rm -r ${sitepack_output_package_path}"
    rm -r "${sitepack_output_package_path}"
fi

# Pack ./{{.SiteFolder}} and .{{.HTTPDotYAML}} into a new sitepack package.
# This will be a file store not a reader.
echo ""
(( step += 1 ))
echo "STEP ${step}:"
echo "WRITE THE ${sitepack_package_name} PACKAGE SOURCE CODE."
echo " * The ${sitepack_package_name} package will be a file store"
echo "     containing all of the files in the /{{.SiteFolder}} folder."
echo " * So this process could take a while."
echo " * See func serveFileStore in {{.ServeDotGo}}."
echo "cd ${appwd}"
cd ..
echo "kickpack -nu -strict -o=${sitepack_output_package_path} ./{{.SiteFolder}} ./{{.HTTPDotYAML}}"
kickpack -nu -strict -o="${sitepack_output_package_path}" ./{{.SiteFolder}} ./{{.HTTPDotYAML}}
if [ $? -gt 0 ]
then
    echo "Oops! That shouldn't happen."
    exit
fi

# Build the new site pack package.
echo " * Success. The ${sitepack_package_name} package source code has been written."

echo ""
(( step += 1 ))
echo "STEP ${step}:"
echo "BUILD THE ${sitepack_package_name} PACKAGE."
echo " * This process could take use of all of your CPU cores."
echo " * This process will take a while."
echo "cd ${sitepack_output_package_path}"
cd "$sitepack_output_package_path"
echo "go build"
go build
if [ $? -gt 0 ]
then
    echo "Oops! Your code is broken."
    exit
fi

echo " * Success."
echo " * You have sucessfully compiled the ${sitepack_package_name} package object code."
echo " * In other words, you have compiled your entire renderer process into object code."

echo ""
(( step += 1 ))
echo "STEP ${step}:"
echo "BUILD THE ENTIRE APPLICATION INTO A SINGLE EXECUTABLE ${prefix}."
echo "   You will do so with the following 2 commands..."
echo "   cd .."
echo "   go build"
`
