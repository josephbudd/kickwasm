...
type {{.Store}}Storer interface {
	...
	Get{{.Store}}(id uint64) (r *types.{{.Store}}Record, err error)
	...
	Get{{.Store}}s() (rr []*types.{{.Store}}Record, err error)
