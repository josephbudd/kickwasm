...
// BoltDatabaseErrorsGo is the mainprocess/ports/boltdatabase/errors.go template.
const BoltDatabaseErrorsGo = `package boltdatabase
...
// BoltDatabaseRepoGo is the mainprocess/ports/boltdatabase/<repo name>.go template.
const BoltDatabaseRepoGo = `package boltdatabase
...
// {{.Repo}}BoltDB implements {{.Repo}}RepoI
...
// Get{{.Repo}} retrieves the records.{{.Repo}}Record from the db.
// Param id [in] is the record id.
// Returns the record and error.
func ({{call .LowerCamelCase .Repo}}db *{{.Repo}}BoltDB) Get{{.Repo}}(id uint64) (*records.{{.Repo}}Record, error) {
	var r records.{{.Repo}}Record
...
// Get{{.Repo}}s retrieves all of the records.{{.Repo}}Record from the db.
// If there are no records.{{.Repo}}Records in the db then it calls {{call .LowerCamelCase .Repo}}db.initialize().
// See {{call .LowerCamelCase .Repo}}db.initialize().
// Returns the records and error.
func ({{call .LowerCamelCase .Repo}}db *{{.Repo}}BoltDB) Get{{.Repo}}s() ([]*records.{{.Repo}}Record, error) {
...
func ({{call .LowerCamelCase .Repo}}db *{{.Repo}}BoltDB) get{{.Repo}}s() ([]*records.{{.Repo}}Record, error) {
	rr := make([]*records.{{.Repo}}Record, 0, 5)
	er := {{call .LowerCamelCase .Repo}}db.DB.View(func(tx *bolt.Tx) error {
		bucketname := []byte({{call .LowerCamelCase .Repo}}BucketName)
		bucket := tx.Bucket(bucketname)
		if bucket != nil {
...
// Returns the error.
func ({{call .LowerCamelCase .Repo}}db *{{.Repo}}BoltDB) Update{{.Repo}}(r *records.{{.Repo}}Record) error {
...
// updates the records.{{.Repo}}Record in the database.
// Param record [in-out] the record to be updated
func ({{call .LowerCamelCase .Repo}}db *{{.Repo}}BoltDB) update{{.Repo}}Bucket(r *records.{{.Repo}}Record) error {
	return {{call .LowerCamelCase .Repo}}db.DB.Update(func(tx *bolt.Tx) error {
		bucketname := []byte({{call .LowerCamelCase .Repo}}BucketName)
