...
// FilePathsFilePathsGo is the mainprocess/data/filepaths/filepaths.go template.
const FilePathsFilePathsGo = `package filepaths
...
func buildUserHomeDataPath() {
	...
	if testing {
		userHomeDataPath = filepath.Join(home, "{{.ApplicationName}}_kga_tests")
	} else {
		userHomeDataPath = filepath.Join(home, ".{{.ApplicationName}}_kga")
	}
